        ...
        public (bool status, int blockCount) AddFile(string url) {
            lock (listOperation) {
                if (fileList.ContainsKey(url)) {
                    ...
                    fileList.Add(url, new FileReaderItem() { fs = cache, BlockSize = FILE_BLOCK_SIZE, BlockCount = length / FILE_BLOCK_SIZE, LastBlockLength = length % FILE_BLOCK_SIZE, UsageCount = 1 });
                    return (true, fileList[url].BlockCount);
                }
            }
        }
